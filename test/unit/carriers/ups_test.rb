...
class UPSTest < Minitest::Test
  ...
  def test_tracking_info
    mock_response = xml_fixture('ups/example_tracking_response')
  ...
  def test_void_shipment
    mock_response = xml_fixture('ups/example_void_response')
    @carrier.expects(:commit).returns(mock_response)
    response = @carrier.void_shipment( "",["1ZISDE016691676846"],{:test => true, :key => "FD0391730BAC7F28",:login => 'SuperPedestrian1', :password => "84Hamilton", :origin_account => "#TODO"})
    assert_equal "Voided", response[:status_code_description]
  end
